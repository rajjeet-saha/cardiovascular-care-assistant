...
// Development/demo patient — switchable at runtime (see setPatientId).
// Precedence: ?patient=P004 URL param > saved choice (localStorage) > P001.
let PATIENT_ID = "P001";
try {
  const __params = new URLSearchParams(window.location.search);
  PATIENT_ID = String(
    __params.get("patient") || localStorage.getItem("cca.patientId") || "P001"
  ).toUpperCase();
} catch (e) { /* storage unavailable — fall back to default */ }

/** Current patient ID (live — all pages poll this patient). */
function getPatientId() { return PATIENT_ID; }

/**
 * Switch the active patient everywhere. Persists across pages in this
 * browser (dashboard, patient app, simulator all follow on next load).
 * Returns false for invalid IDs.
 */
function setPatientId(id) {
  const v = String(id || "").trim().toUpperCase();
  if (!/^[A-Z0-9_-]{1,16}$/.test(v)) return false;
  PATIENT_ID = v;
  try { localStorage.setItem("cca.patientId", PATIENT_ID); } catch (e) {}
  return true;
}
